...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .cat { margin-left: auto; font-size: .7rem; font-weight: 600; color: var(--accent); background: var(--accent-dim); padding: .2rem .5rem; border-radius: 4px; text-transform: uppercase; letter-spacing: .05em; white-space: nowrap; }
...
function render() {
  ...
  for (const s of filtered) {
    ...
    // Resolve icon name from compose directory or lowercase name with custom overrides
    const customIcons = {
      "tandoor": "tandoor-recipes",
      "tubearchivist": "tube-archivist",
      "lidarr-on-steroids": "lidarr",
    ...
    let iconName = s.compose || s.name.toLowerCase().replace(/[^a-z0-9]/g, "");
    if (customIcons[iconName]) {
      iconName = customIcons[iconName];
    }

    card.innerHTML = \`
      <div class="card-head">
        <img class="card-icon" src="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/\${iconName}.png" onerror="this.style.display='none'">
        <span class="name">\${nameLink}</span>
